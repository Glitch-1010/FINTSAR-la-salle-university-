...
```{r financialLiteracySurvey ,echo = FALSE}
library(readr)
...
(please make a bar chart and mean and std dev as well)
## Bar Graph
```{r table}
library(tidyverse)
...
## Dataset Visualization
```{r RelationperCapitaGDPAdultDeposit ,echo = FALSE}
    library(readr)
...
$$m = \frac {sum of terms} {number of terms}$$
```{r mea}
x = mean(data$Log_of_GDP_per_capita)
...
data2$Country <- as.factor(data$Country)
